...
class QueueManager:
    ...
    def receive_from_prioritizer_n(self, queue_id: int, num_messages=1):
        self._check_priority_id_in_range(queue_id)
    ...
    def receive_from_worker_n(self, queue_id: int, num_messages=1):
        self._check_worker_id_in_range(queue_id)
        return self.worker_queues[queue_id].receive_messages(
            MaxNumberOfMessages=num_messages
        )

    def send_to_worker_n(self, queue_id, message_body):
        self._check_worker_id_in_range(queue_id)
        self.worker_queues[queue_id].send_message(MessageBody=message_body)

    def delete_from_worker_n(
        self, queue_id: int, msg_ids: list[int], receipt_ids: list[int]
    ):
        self._check_worker_id_in_range(queue_id)
        entries = self._build_delete_request(msg_ids, receipt_ids)
        if len(entries) > 0:
            self.worker_queues[queue_id].delete_messages(Entries=entries)

    def _create_prioritizer_queues(self):
        queues_by_id = {}
        for qconfig in config.prioritizer_queue_configs:
            queues_by_id[qconfig.queue_config.queue_id] = self.sqs.Queue(
                qconfig.queue_config.url
            )
    ...
    def _create_worker_queues(self):
        queues_by_id = {}
        for qconfig in config.worker_queue_configs:
            queues_by_id[qconfig.queue_id] = self.sqs.Queue(qconfig.url)

        return queues_by_id

    def _check_priority_id_in_range(self, queue_id: int):
        if queue_id not in self.prioritizer_queues:
            raise ValueError("Id must reference prioritizer queues 1...n")

    def _check_worker_id_in_range(self, queue_id: int):
        if queue_id not in self.worker_queues:
            raise ValueError("Id must reference worker queues 1...n")
